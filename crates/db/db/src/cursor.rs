...
pub fn build_cursor_conditions(
    ...
    if let Some(values) = cursor_values {
        ...
        for (i, (ob, val)) in pagination.order_by.iter().zip(values).enumerate() {
            ...
            // For now, use placeholder - this cursor system needs to be rewritten for PostgreSQL
            let condition = if i == 0 {
                format!("{} {} $PARAM", ob.field, operator)
            } else {
                let prev = (0..i)
                    .map(|j| {
                        let prev_ob = &pagination.order_by[j];
                        format!("{} = $PARAM", prev_ob.field)
                    })
                    .collect::<Vec<_>>()
                    .join(" AND ");
                format!("({} AND {} {} $PARAM)", prev, ob.field, operator)
            };
...
pub fn build_cursor_values(pagination: &Pagination, row: &PgRow) -> Result<Vec<String>, Error> {
