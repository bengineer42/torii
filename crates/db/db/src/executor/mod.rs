...
use serde_json;
use sqlx::{FromRow, Pool, Postgres, Transaction as SqlxTransaction};
...
pub struct Executor<'c, P: Provider + Sync + Send + Clone + 'static> {
    ...
    pool: Pool<Postgres>,
    transaction: Option<SqlxTransaction<'c, Postgres>>,
    ...
    async fn handle_query_message(&mut self, query_message: QueryMessage) -> QueryResult<()> {
        ...
        match query_message.query_type {
            QueryType::UpdateCursors(update_cursors) => {
                ...
                let mut contracts: Vec<torii_db_types::Contract> =
                ...
                for cursor in &mut contracts {
                    ...
                    sqlx::query(
                        "UPDATE contracts SET head = $1, last_block_timestamp = $2, \
                         last_pending_block_tx = $3, updated_at = CURRENT_TIMESTAMP WHERE id = \
                         $4",
                    )
                    .bind(cursor.head)
            ...
            QueryType::StoreTransaction(store_transaction) => {
                ...
                let mut transaction =
                    torii_db_types::Transaction::from_row(&query.fetch_one(&mut **tx).await?)?;

                for contract_address in &store_transaction.contract_addresses {
                    sqlx::query(
                        "INSERT INTO transaction_contract (transaction_hash, \
                         contract_address) VALUES ($1, $2) ON CONFLICT DO NOTHING",
                    )
                ...
                for unique_model in &store_transaction.unique_models {
                    sqlx::query(
                        "INSERT INTO transaction_models (transaction_hash, \
                         model_id) VALUES ($1, $2) ON CONFLICT DO NOTHING",
                    )
                    .bind(transaction.transaction_hash.clone())
            ...
            QueryType::SetEntity(entity) => {
                ...
                let mut entity_updated = torii_db_types::Entity::from_row(&row)?;
            ...
            QueryType::RegisterModel => {
                let row = query.fetch_one(&mut **tx).await?;
                let model_registered = torii_db_types::Model::from_row(&row)?;
            ...
            QueryType::RegisterContract => {
                let row = query.fetch_one(&mut **tx).await?;
                let contract_registered = torii_db_types::Contract::from_row(&row)?;
            ...
            QueryType::EventMessage(em_query) => {
                ...
                let mut event_message = torii_db_types::Entity::from_row(&event_messages_row)?;
            ...
            QueryType::StoreEvent => {
                let row = query.fetch_one(&mut **tx).await?;
                let event = torii_db_types::Event::from_row(&row)?;
