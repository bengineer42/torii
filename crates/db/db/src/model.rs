...
impl ModelSQLReader {
    pub async fn new(selector: Felt, pool: Pool<Postgres>) -> Result<Self, Error> {
...
impl ModelReader<Error> for ModelSQLReader {
    ...
    async fn schema(&self) -> Result<Ty, Error> {
        let schema: String = sqlx::query_scalar("SELECT schema FROM models WHERE id = $1")
    ...
    async fn packed_size(&self) -> Result<u32, Error> {
        Ok(self.packed_size as u32)
    }

    async fn unpacked_size(&self) -> Result<u32, Error> {
        Ok(self.unpacked_size as u32)
    }
    ...
    match ty {
        Ty::Primitive(primitive) => {
            match &primitive {
                ...
                Primitive::U8(_) => {
                    let value = row.try_get::<i16, &str>(column_name)?;
                    primitive.set_u8(Some(value as u8))?;
                }
                Primitive::U16(_) => {
                    let value = row.try_get::<i32, &str>(column_name)?;
                    primitive.set_u16(Some(value as u16))?;
                }
                Primitive::U32(_) => {
                    let value = row.try_get::<i64, &str>(column_name)?;
                    primitive.set_u32(Some(value as u32))?;
                }
                Primitive::U64(_) => {
                    let value = row.try_get::<String, &str>(column_name)?;
                    let hex_str = value.trim_start_matches("0x");
