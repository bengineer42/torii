...
impl ReadOnlyStorage for Sql {
    ...
    async fn model(&self, selector: Felt) -> Result<Model, StorageError> {
        ...
        let model_metadata = Model {
            ...
            schema,
            packed_size: model.packed_size as u32,
            unpacked_size: model.unpacked_size as u32,
    ...
    async fn contracts(&self, query: &ContractQuery) -> Result<Vec<Contract>, StorageError> {
        ...
        let mut query = sqlx::query_as::<_, torii_db_types::Contract>(&query_builder);
        for value in bind_values {
            query = query.bind(value);
        }
        ...
        let items: Vec<TokenBalance> = page
            ...
            .map(|row| {
                Result::<TokenBalance, Error>::Ok(
                    torii_db_types::TokenBalance::from_row(&row)?.into(),
                )
            })
            .collect::<Result<Vec<_>, _>>()?;
        Ok(Page {
        ...
        let items: Vec<TokenContract> = page
            ...
            .map(|row| {
                Result::<TokenContract, Error>::Ok(
                    torii_db_types::TokenContract::from_row(&row)?.into(),
                )
            })
            .collect::<Result<Vec<_>, _>>()?;
        Ok(Page {
    ...
    async fn transactions(
        ...
        let headers: Vec<torii_db_types::Transaction> = page
            .items
            .into_iter()
            .map(|row| {
                Result::<torii_db_types::Transaction, Error>::Ok(
                    torii_db_types::Transaction::from_row(&row)?,
                )
            })
            .collect::<Result<Vec<_>, _>>()?;
        ...
        for header in headers {
            ...
            let transaction = Transaction {
                ...
                nonce: Felt::from_str(&header.nonce)
                    .map_err(|e| Error::Parse(ParseError::FromStr(e)))?,
                block_number: header.block_number as u64,
        ...
        let items: Vec<TokenTransfer> = page
            ...
            .map(|row| {
                Result::<TokenTransfer, Error>::Ok(
                    torii_db_types::TokenTransfer::from_row(&row)?.into(),
                )
            })
            .collect::<Result<Vec<_>, _>>()?;
    ...
    async fn aggregations(
        ...
        let items: Vec<AggregationEntry> = page
            ...
            .into_iter()
            .map(|row| {
                let aggregation = torii_db_types::AggregationEntryWithPosition::from_row(&row)?;
    ...
    async fn activities(&self, query: &ActivityQuery) -> Result<Page<Activity>, StorageError> {
        ...
        let items: Vec<Activity> = page
            ...
            .into_iter()
            .map(|row| {
                Result::<Activity, Error>::Ok(torii_db_types::Activity::from_row(&row)?.into())
            })
            .collect::<Result<Vec<_>, _>>()?;
        Ok(Page {
            items,
...
impl Storage for Sql {
    ...
    async fn register_contract(
        ...
        let insert_contract = "INSERT INTO contracts (id, contract_address, contract_type, head, updated_at, created_at) VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) ON CONFLICT(id) DO UPDATE SET contract_type=EXCLUDED.contract_type, head=EXCLUDED.head, updated_at=CURRENT_TIMESTAMP RETURNING *";
